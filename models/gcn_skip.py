...
class GCNTwoLayersSkipConnection(tf.keras.Model):
    ...
    def call(self, inputs: Tuple[tf.SparseTensor, tf.Tensor], training=None):
        adj, nodes = inputs
        # print(f"[GCNTwoLayersSkipConnection] Input nodes shape: {nodes.shape}")
        x = self.layer_gcn([adj, nodes])
        # print(f"[GCNTwoLayersSkipConnection] Output after GCNLayer: {x.shape}")
        if self.dropout is not None:
            x = self.dropout(x, training=training)
            # print(f"[GCNTwoLayersSkipConnection] Output after dropout: {x.shape}")
        x = self.layer_gcn_skip([adj, x, nodes])
        # print(f"[GCNTwoLayersSkipConnection] Output after skip connection: {x.shape}")
        return x
